...
class HybridRecommenderSystem:
    ...
    def __calculate_content_based_similarities(
        self, song_name, artist_name, songs_data, transformed_matrix
    ):
        ...
        if song_row.empty:
            raise ValueError("Song not found in songs_data")
    ...
    def __calculate_collaborative_filtering_similarities(
        self, song_name, artist_name, track_ids, songs_data, interaction_matrix
    ):
        song_row = songs_data.loc[
            (songs_data["name"] == song_name) &
            (songs_data["artist"] == artist_name)
        ...
        if input_track_id not in track_ids:
            raise ValueError("Track ID not found in collaborative data")

        input_index = np.where(track_ids == input_track_id)[0].item()

        input_vector = interaction_matrix[input_index]

        raw_cf_scores = cosine_similarity(
            input_vector, interaction_matrix
        )

        # 🔥 ALIGN CF SCORES WITH songs_data ORDER
        aligned_cf_scores = np.zeros((1, len(songs_data)))

        track_id_to_index = {
            tid: idx for idx, tid in enumerate(track_ids)
        }

        for i, tid in enumerate(songs_data["track_id"]):
            if tid in track_id_to_index:
                aligned_cf_scores[0, i] = raw_cf_scores[
                    0, track_id_to_index[tid]
                ]

        return aligned_cf_scores
    ...
    def __weighted_combination(
        self, content_based_scores, collaborative_filtering_scores
    ):
        return (
    ...
    def give_recommendations(
        ...
        # Content-based
        content_scores = self.__calculate_content_based_similarities(
            song_name, artist_name, songs_data, transformed_matrix
        )

        # Collaborative
        cf_scores = self.__calculate_collaborative_filtering_similarities(
            song_name, artist_name, track_ids, songs_data, interaction_matrix
        )

        # Normalize
        norm_content_scores = self.__normalize_similarities(content_scores)
        norm_cf_scores = self.__normalize_similarities(cf_scores)

        # Weighted hybrid score
        hybrid_scores = self.__weighted_combination(
            norm_content_scores, norm_cf_scores
        )

        # Sort & select top-k
        top_indices = np.argsort(hybrid_scores.ravel())[
            -self.number_of_recommendations - 1:
        ][::-1]

        top_track_ids = songs_data.iloc[top_indices]["track_id"].values
        top_scores = hybrid_scores.ravel()[top_indices]
